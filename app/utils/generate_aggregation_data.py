...
def prepare_df_for_calculations(irrigation_reports: List[IrrigationOperation]) -> pd.DataFrame:
    data_for_df = []
    for irrig in irrigation_reports:
        data_for_df.append({"Started Date": irrig.hasStartDatetime,
                            "Dose": irrig.hasAppliedAmount.numericValue if irrig.hasAppliedAmount else 0})
...
def generate_total_volume_graph(df: pd.DataFrame, parcel_area: int) -> io.BytesIO:
    ...
    for i, txt in enumerate(df['Total Volume']):
        plt.annotate(txt, (df['Started Date'].iloc[i], df['Total Volume'].iloc[i]),
...
def generate_amount_per_hectare(df: pd.DataFrame) -> io.BytesIO:
    ...
    for i, txt in enumerate(df['Dose']):
        plt.annotate(txt, (df['Started Date'].iloc[i], df['Dose'].iloc[i]),
                     textcoords="offset points", xytext=(0, 5), ha='center')
